...
describe.me <- function(dataset, cols, rows, scale) {
  ...
  highest.frequency.rounded <- round_any(highest.frequency, 10, ceiling) + 50
  ...
  scale.describe <- knitr::kable(as.data.frame(psych::describe(dataset)), label = "Descriptives", digits = 2)
